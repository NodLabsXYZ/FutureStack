...
const checkLocalFiles = async () => {
  ...
  if (generalUploadStore('metadataFileNames') || generalUploadStore('manifests')) {
    ...
    if (generalUploadStore('manifests')) {
      console.log(generalUploadStore('manifests'))
    }

    const success = await createAssets(project, assets)
