...
export default function NftObjectGrid(props: NftObjectGridProps) {
    ...
    return (
        <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {nftObjects.map((nftObj: NftObject) => (
                <li
                    key={nftObj.preview}
                ...
                >
                    ...
                    >
                        <img className="w-32 h-32 flex-shrink-0 mx-auto rounded-lg" src={nftObj.preview} alt={nftObj.imageFile.name} />
                        <h3 className="mt-6 text-gray-900 text-sm font-medium">{nftObj.name}</h3>
